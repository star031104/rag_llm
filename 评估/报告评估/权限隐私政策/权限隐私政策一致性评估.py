...
4. 导出总体混淆矩阵
5. 导出每个 APP 的评估结果
6. 导出每个标签的 Precision / Recall / F1
7. 导出错误样本
8. 导出标签分布
9. 生成论文可直接引用的 summary.json
"""
...
# 直接在这里改路径
# =========================================================
INPUT_FILE = Path(r"permission_dataset.csv")
OUTPUT_DIR = Path(r"eval_outputs")
SHEET_NAME = 0   # 如果是 xlsx，可改成工作表名或索引；csv 时无效
# =========================================================

LABELS: List[str] = ["已明确告知", "间接告知", "未告知"]
REQUIRED_COLUMNS: List[str] = ["app_name", "permission", "pred_label", "gold_label"]
...
def prepare_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 删除关键字段为空的样本
    out = out[(out["app_name"] != "") & (out["permission"] != "") & (out["pred_label"] != "") & (out["gold_label"] != "")]
...
def build_per_app_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows).sort_values(by=["sample_count", "app_name"], ascending=[False, True]).reset_index(drop=True)
